...
class MainApp(tk.Frame, EnigmaMachine):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Attributes:
            button_list (list): list of commutator (plugboard) buttons that will remain in a commutator window
            button_list_updated (list): list of updated commutator buttons that will be added to the rest in the window.
    ...
    def initUI(self):
        ...
        self.master.grid_propagate('False') # Slaves do not determine the size of the master widget
        ...
        #Direct IO
        self.direct_label = tk.LabelFrame(self.translator_board, text='Direct', padx=5, pady=5)
        self.direct_label.grid(column=0, row=0, rowspan=3, padx=5, pady=5, sticky='nsew')
        ...
        direct_scroller = ttk.Scrollbar(self.direct_label, command=self.direct_input.yview)
        direct_scroller.grid(column=1, row=0, sticky='NSE')
        self.direct_input['yscrollcommand'] = direct_scroller.set
        
        #Encrypted IO
        self.encrypted_label = tk.LabelFrame(self.translator_board, text='Encrypted', padx=5, pady=5)
        self.encrypted_label.grid(column=2, row=0, rowspan=3, padx=5, pady=5, sticky='nsew')
        ...
        self.encrypted_input.grid(column=0, row=0, padx=0, pady=0, sticky='nsew')
        encrypted_scroller = ttk.Scrollbar(self.encrypted_label, command=self.encrypted_input.yview)
        ...
        #Commutator
        self.comm_button = ttk.Button(left_button_box, text='Open commutator settings')
        ...
        positioning_frame=tk.LabelFrame(baseboard, height=50, width=250, text='Current Rotor Position')
        positioning_frame.grid(column=3, row=0, sticky='e')
        
        self.var1 = tk.IntVar()

        self.position_1 = ttk.Spinbox(positioning_frame, textvariable=self.var1, from_=0, to=25, width=3, command=self.setWheels)
        self.position_1.set(self.wheel1pos)
        self.position_1.grid(column=0, row=0, sticky='nsew', padx=5, pady=5)
        
        self.position_2 = ttk.Spinbox(positioning_frame, from_=0, to=25, width=3, command=self.setWheels)
        self.position_2.set(self.wheel2pos)
        self.position_2.grid(column=1, row=0, sticky='nsew', padx=5, pady=5)
        
        self.position_3 = ttk.Spinbox(positioning_frame, from_=0, to=25, width=3, command=self.setWheels)
        self.position_3.set(self.wheel3pos)
        self.position_3.grid(column=2, row=0, sticky='nsew', padx=5, pady=5)
        
        #Rotors Selected
        rotors_frame = tk.LabelFrame(baseboard, text='Rotors', width=250, padx=5)
        ...
        self.rotor1 = ttk.OptionMenu(rotors_frame, self.rotor1_value, rotor_options[0], *rotor_options)
        ...
        self.rotor2 = ttk.OptionMenu(rotors_frame, self.rotor2_value, rotor_options[0], *rotor_options)
        ...
        self.rotor3 = ttk.OptionMenu(rotors_frame, self.rotor3_value, rotor_options[0], *rotor_options)
    ...
    def setWheels(self):
        self.rotor_set = str(self.rotor1_value.get()) + str(self.rotor2_value.get()) + str(self.rotor3_value.get())
        self.positionSetter([int(self.position_1.get()), int(self.position_2.get()), int(self.position_3.get())])
    ...
    def openCommSettings(self):
        ...
        self.couple_entry = tk.Entry(self.control_frame, width=4, textvariable=self.couple_var)
        ...
        self.button_creator() #init exising links as buttons when the comm_board is open:
    ...
    def button_creator(self):
        ...
        for pair in self.plugboard:
            ...
            self.button_link = ttk.Button(self.comm_window, text=f'{couple}', command=lambda: self.on_linkbutton_click(couple))
